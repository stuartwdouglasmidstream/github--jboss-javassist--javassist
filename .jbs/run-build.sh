...
export PROJECT_VERSION=3.29.0-GA
...
if [ -n "" ]; then
  echo "Setting version to 3.29.0-GA to match enforced version"
  mvn -B -e -s "/root/software/settings/settings.xml" -t "/root/software/settings/toolchains.xml" org.codehaus.mojo:versions-maven-plugin:2.8.1:set -DnewVersion="3.29.0-GA" | tee /root/project/logs/enforce-version.log
fi
